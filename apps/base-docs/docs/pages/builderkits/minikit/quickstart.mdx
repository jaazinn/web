...
This guide shows you how to get started with MiniKit to create mini apps, also known as frames, right in the social feed! It can also be used to update an exisitng standalone app to a mini app too. We'll start by setting up the template project with the CLI tool and then explore both built-in and additional features of MiniKit.
...
The Frame Manifest is required in order for users to save the frame to their account. This means its also required to send notifications to the user. We initially skipped this step when setting up the app. Now that we know our ngrok address, we can configure our manifest.
...
Once connected, add the ngrok url and sign the manifest.

:::warning[Developement Pitfall]
Note that the ngrok url will change every time you restart ngrok. This means you'll need to recreate the manifest each time you spin up ngrok since its dependent on the URL.
:::

### useAddFrame

The `useAddFrame` hook is used to add your frame to the user's list of frames. It's implemented in your main page component and displays a button in the top right allowing the user to add the frame to their list.

When a user adds the frame, it returns a url and token, which is used for sending the user notifications. For this walkthrough we'll simply console.log those results to use them later when setting up notifications.
